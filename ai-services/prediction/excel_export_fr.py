...
class StrategicAnalysisExcelExporterFR:
    def __init__(self):
    ...
    def _create_summary_sheet(self, data: Dict[str, Any]):
        ...
        ws = self.workbook.create_sheet("Évaluation Stratégique")
        
        # Title
        ws['A1'] = "UPLINK 5.0 - Rapport d'Analyse Stratégique"
        ...
        ws['B3'] = data.get('project_title', 'Projet Sans Titre')
        ...
        row += 1
        ws[f'A{row}'] = data.get('executive_summary_fr', data.get('executive_summary', 'Aucun résumé disponible.'))
        ws.merge_cells(f'A{row}:D{row+3}')
        ws[f'A{row}'].alignment = Alignment(wrap_text=True, vertical='top')
        
        # Key Dimensions Table
        row += 5
        ws[f'A{row}'] = "Dimensions Clés"
        ws[f'A{row}'].font = Font(bold=True, size=12)
        row += 1
        ...
        dim_data = [
            ("Dimension", "Score"),
            ("Probabilité de Succès", f"{dimensions.get('success_probability', 0):.1f}"),
            ("Adéquation au Marché", f"{dimensions.get('market_fit', 0):.1f}"),
            ("Préparation à l'Exécution", f"{dimensions.get('execution_readiness', 0):.1f}"),
            ("Préparation aux Investisseurs", f"{dimensions.get('investor_readiness', 0):.1f}"),
            ("Durabilité Financière", f"{dimensions.get('financial_sustainability', 0):.1f}")
        ]
    ...
    def _create_ici_sheet(self, data: Dict[str, Any]):
        ...
        ws = self.workbook.create_sheet("Analyse ICI")
        
        # Title
        ws['A1'] = "Indice de Confiance en l'Innovation (ICI)"
        ws['A1'].font = self.title_font
        ...
        row = 6
        ws[f'A{row}'] = "Détail des Dimensions"
        ws[f'A{row}'].font = Font(bold=True, size=12)
        row += 1
        ...
        dim_list = [
            ...
            ("Probabilité de Succès", dimensions.get('success_probability', 0), "20%"),
            ("Adéquation au Marché", dimensions.get('market_fit', 0), "20%"),
            ("Préparation à l'Exécution", dimensions.get('execution_readiness', 0), "20%"),
            ("Préparation aux Investisseurs", dimensions.get('investor_readiness', 0), "20%"),
            ("Durabilité Financière", dimensions.get('financial_sustainability', 0), "20%")
        ]
    ...
    def _create_insights_sheet(self, data: Dict[str, Any]):
        ...
        ws = self.workbook.create_sheet("Perspectives Dirigeants")
        
        # Title
        ws['A1'] = "Perspectives pour les Dirigeants"
        ws['A1'].font = self.title_font
        ws.merge_cells('A1:D1')
        
        # Header row
        row = 3
        headers = ["#", "Perspective", "Impact", "Priorité"]
    ...
    def _create_roadmap_sheet(self, data: Dict[str, Any]):
        ...
        ws = self.workbook.create_sheet("Feuille de Route")
        
        # Title
        ws['A1'] = "Feuille de Route Actionnable"
        ws['A1'].font = self.title_font
        ws.merge_cells('A1:E1')
        
        # Header row
        row = 3
        headers = ["Étape", "Titre", "Description", "Calendrier", "Priorité"]
        for col_idx, header in enumerate(headers, start=1):
            cell = ws.cell(row=row, column=col_idx)
            cell.value = header
            cell.fill = self.header_fill
    ...
    def _create_investment_sheet(self, data: Dict[str, Any]):
        ...
        ws = self.workbook.create_sheet("Analyse Investissement")
        
        # Title
        ws['A1'] = "Analyse d'Investissement"
        ws['A1'].font = self.title_font
        ws.merge_cells('A1:D1')
    ...
    def _create_critical_path_sheet(self, data: Dict[str, Any]):
        ...
        ws = self.workbook.create_sheet("Chemin Critique")
        
        # Title
        ws['A1'] = "Chemin Critique vers le Succès"
        ws['A1'].font = self.title_font
        ws.merge_cells('A1:D1')
        
        # Header row
        row = 3
        headers = ["Étape", "Titre", "Description", "Durée"]
        for col_idx, header in enumerate(headers, start=1):
            cell = ws.cell(row=row, column=col_idx)
            cell.value = header
            cell.fill = self.header_fill
...
def export_to_excel_fr(analysis_data: Dict[str, Any], output_path: str) -> str:
    """
    Main function to export strategic analysis to French Excel
    ...
    Returns:
        Path to generated Excel file
    """
    exporter = StrategicAnalysisExcelExporterFR()
...
if __name__ == "__main__":
    ...
    output = export_to_excel_fr(sample_data, '/tmp/test_report_fr.xlsx')
    print(f"✅ French Excel generated: {output}")
