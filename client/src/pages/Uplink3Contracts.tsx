// Added for Flowchart Match - UPLINK3 Smart Contracts Page
...
export default function Uplink3Contracts() {
  const { user } = useAuth();
  ...
  const createMutation = trpc.uplink3.contracts.create.useMutation({
    onSuccess: () => {
  ...
  const signMutation = trpc.uplink3.contracts.sign.useMutation({
    onSuccess: () => {
