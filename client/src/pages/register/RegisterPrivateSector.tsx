...
export default function RegisterPrivateSector() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Private Sector registration:", formData);
      ...
      <div className="max-w-3xl mx-auto py-12">
        <Card className="p-8 bg-slate-900/50 border-blue-500/20">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">تسجيل قطاع خاص</h1>
              <p className="text-slate-400">انضم كمؤسسة من القطاع الخاص</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <Label className="text-white">اسم المؤسسة *</Label>
                <Input
                  required
              ...
              <div>
                <Label className="text-white">نوع المؤسسة *</Label>
                <Input
                  required
                  value={formData.organizationType}
                  onChange={(e) => setFormData({ ...formData, organizationType: e.target.value })}
                  className="bg-slate-800/50 border-slate-700 text-white"
                  placeholder="مثال: مؤسسة، جمعية، منظمة"
                />
              </div>
            ...
            <div>
              <Label className="text-white">نبذة عن المؤسسة *</Label>
              <Textarea
                required
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                className="bg-slate-800/50 border-slate-700 text-white min-h-[120px]"
                placeholder="أهداف المؤسسة ومجالات العمل..."
              />
            </div>
