...
export default function Uplink2Projects() {
  const params = useParams<{ id: string }>();
  const projectId = Number(params.id);
  const [, setLocation] = useLocation();
  const { toast } = useToast();

  const { data: project, isLoading } = trpc.uplink2.getProjectById.useQuery(
    { projectId },
    { enabled: !!projectId }
  );

  const promoteToUplink3Mutation = trpc.uplink2.promoteToUplink3.useMutation({
    onSuccess: (data) => {
      toast({
        title: "تم بنجاح!",
        description: "تم نقل مشروعك إلى UPLINK 3 للبيع",
      });
  ...
  return (
    <div className="container mx-auto py-12">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Promote to UPLINK 3 */}
        <Card className="glass-card p-8">
          <h2 className="text-2xl font-bold text-white mb-4">الانتقال إلى UPLINK 3</h2>
          <p className="text-gray-300 mb-6">
            بعد نجاح المطابقة والاتفاق مع الشركاء، يمكنك الانتقال إلى UPLINK 3 للبيع أو
            الاستحواذ.
          </p>
          <Button
            onClick={() => promoteToUplink3Mutation.mutate({ projectId })}
            disabled={promoteToUplink3Mutation.isPending}
            className="w-full bg-purple-600 hover:bg-purple-700"
          >
            {promoteToUplink3Mutation.isPending ? (
              <>
