// Added for Flowchart Match - UPLINK3 Escrow Management Page
...
export default function Uplink3Escrow() {
  const { user } = useAuth();

  const { data: stats } = trpc.uplink3.escrow.getStats.useQuery(undefined, {
    enabled: !!user
  });

  const depositMutation = trpc.uplink3.escrow.deposit.useMutation({
    onSuccess: () => {
