...
export default function Naqla3ContractDetail() {
  ...
  const { data: milestones, isLoading, refetch } = trpc.naqla3.milestones.getContractMilestones.useQuery(
    { contractId, blockchainContractId },
    { enabled: !!user && contractId > 0 }
  );

  const startMutation = trpc.naqla3.milestones.start.useMutation({
    onSuccess: (data) => {
  ...
  const completeMutation = trpc.naqla3.milestones.complete.useMutation({
    onSuccess: (data) => {
      toast.success(data.message);
      toast.info(`Transaction: ${data.transactionHash.slice(0, 10)}...`);
      refetch();
  ...
  const approveMutation = trpc.naqla3.milestones.approve.useMutation({
    onSuccess: (data) => {
      toast.success(data.message);
      toast.info(`Transaction: ${data.transactionHash.slice(0, 10)}...`);
      refetch();
  ...
  const rejectMutation = trpc.naqla3.milestones.reject.useMutation({
    onSuccess: (data) => {
      toast.success(data.message);
      toast.info(`Transaction: ${data.transactionHash.slice(0, 10)}...`);
      refetch();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <Button
            variant="ghost"
            onClick={() => navigate('/naqla3/contracts')}
