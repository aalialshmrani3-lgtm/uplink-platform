...
- [x] تدقيق read-only لحفظ backend والبيانات والأمن والاختبارات وطبقة الذكاء بين 7657e44a و2021c85c والأرشيفات المرجعية
- [x] إصلاح مسار static production لخادم Vite كي تخدم الحاوية `dist/public` ويعود النطاق العام من 503
...
- [x] نشر NAQLA3 وإجراء production smoke عام read-only ثم إغلاق قبول المرحلة
...
## NAQLA FINAL PLATFORM RELEASE CANDIDATE

- [x] تنظيف علامات تعارض Git وبقايا التطوير الحاجبة للإصدار دون تغيير السلوك المقفل
- [x] تدقيق سلامة المعمارية والكيانات القانونية وتغطية synthetic-auth وauthorization وprivacy والإلغاء على مستوى المنصة
- [x] إنشاء وتشغيل رحلة HTTP حقيقية معزولة NAQLA1 إلى NAQLA2 إلى NAQLA3 مع قاعدة MariaDB نظيفة وبيانات synthetic وتنظيف مضمون
- [x] مراجعة وإصلاح الواجهات الحرجة للـRTL والـLTR والجوال واللوحي وإتاحة الوصول وحالات التحميل والخطأ والفراغ
- [x] تنفيذ فحوص الإصدار النهائية للأمن والخصوصية والهجرات والبناء والأداء العملي والحزمة وخطة العرض والتشغيل والمخاطر
- [x] إنشاء Release Candidate ونشره وإجراء smoke عام read-only ثم حزم التسليم والاستعادة وبصمات SHA-256
- [x] إزالة الادعاءات العامة غير المدمجة المثبتة في الصفحة الرئيسية وربط عرض المحركات بقدرات NAQLA الفعلية فقط
...
- [x] تنفيذ فحوص القبول والحزم والنشر محلياً فقط مع `AI_EXTERNAL_PROVIDER_ENABLED=false` و`EXTERNAL_AI_DEFERRED`
...
- [x] إنتاج تقرير القبول والحزم والتحقق من النشر الحالي ودخان الإنتاج
- [x] منع cache لصفحات HTML وSPA fallback في النشر حتى يحمّل `uplink5.xyz` bundle checkpoint الحالية
- [x] إضافة اختبار آلي لترويسات HTML وSPA no-store مع بقاء الأصول المتجزئة immutable
...
- [x] منع إعلان READY أو إنشاء حزمة نهائية جديدة قبل نجاح هذه البنود واختبارات الرحلة الكاملة
- [x] تأجيل إعادة النشر إلى ما بعد اكتمال الربط والـonboarding والاختبارات، ثم تنفيذ محاولة نشر واحدة يتبعها smoke
...
- [x] تحديث mock واجهة `/naqla` ليغطي mutation إنشاء المعاملة التجارية من Engagement بعد إضافة الربط الخادمي
