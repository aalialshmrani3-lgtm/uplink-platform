...
export const contracts = mysqlTable("contracts", {
	...
	partyAsignedAt: timestamp("partyASignedAt", { mode: 'string' }),
	partyBsignature: text("partyBSignature"),
	partyBsignedAt: timestamp("partyBSignedAt", { mode: 'string' }),
...
export const events = mysqlTable("events", {
	...
	isVirtual: int("isVirtual").default(0),
...
export const naqla3CommercialAssets = mysqlTable("naqla3_commercial_assets", {
  ...
  tenantId: int(),
  sourceListingId: int(),
  sourceInnovationRecordId: int(),
  sourceInnovationRecordVersion: int(),
  sourceSnapshotHash: varchar({ length: 128 }),
  title: varchar({ length: 500 }).notNull(),
  scope: text().notNull(),
  summary: text(),
  assetType: varchar({ length: 80 }),
  classification: varchar({ length: 40 }),
...
export const naqla3CommercialTransactions = mysqlTable("naqla3_commercial_transactions", {
  ...
  tenantId: int(),
  commercialAssetVersionId: int(),
  origin: varchar({ length: 80 }),
  stage: varchar({ length: 40 }),
  ownerUserId: int(),
  policyVersion: varchar({ length: 64 }),
  startedAt: timestamp({ mode: 'string' }),
  closedAt: timestamp({ mode: 'string' }),
  outcome: varchar({ length: 80 }),
  transitionVersion: int().notNull().default(1),
  idempotencyKey: varchar({ length: 160 }),
  status: mysqlEnum(['initiated', 'human_review', 'contract_ready', 'executing', 'cancelled', 'draft', 'active', 'waiting_on_counterparty', 'needs_information', 'paused', 'completed', 'withdrawn', 'terminated', 'archived']).notNull().default('draft'),
