...
const tempBase = await mkdtemp(path.join(tmpdir(), "naqla2d-mariadb-"));
...
const dbName = `naqla_phase22d_test_${randomBytes(5).toString("hex")}`;
const testUser = "naqla2d";
...
try {
  ...
  const runner = spawn("pnpm", ["tsx", "scripts/phase22d_http_harness.ts"], {
    cwd: projectRoot,
